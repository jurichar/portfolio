...
function Tag({ content }: TagProps) {
  ...
  return (
    <span className={`px-3 py-1 border-white border-2 rounded-full ${color}`}>
      {content}
    </span>
  );
}
...
function Works() {
  ...
  return (
    <div className="text-white min-h-full w-full px-48">
      ...
      <div className="grid grid-cols-2 w-full h-full mb-10">
      ...
      </div>
      {
        isOpen && <motion.div
          ...
          className='h-screen w-[50vw] fixed top-0 left-0 z-[100] bg-black'
          onClick={() => {
            setIsOpen(!isOpen);
            enableScroll();
          }
      ...
      <motion.div
        ...
        className='min-h-screen w-[50vw] fixed top-0 right-0 bottom-0 backdrop-blur-3xl flex flex-col justify-center items-center z-50 p-10 gap-10 overflow-scroll'
